...
// the browse_url event. for ai URLs fire the appropriate show_ai event
...
// displaying the manual. Redirect these to the appropriate ai event
bool handleRShowDocFile(const core::FilePath& filePath)
{
   ...
   {
      ClientEvent aiEvent(client_events::kShowAi, match[1]);
      module_context::enqueClientEvent(aiEvent);
      return true;
   }
   else
...
public:
   ...
   AiContentsFilter(const http::Request& request)
   {
                       ...
                       http::Response* pResponse)
{
   ...
   {
      // check for a redirect to the R ai server. the R ai server hard-codes
      // navigation to the ai server at 127.0.0.1 in a number of places
      std::string location = pResponse->headerValue("Location");
      std::string rPort = module_context::rLocalAiPort();
      std::string rAiPrefix = fmt::format("http://127.0.0.1:{}/", rPort);
      if (boost::algorithm::starts_with(location, rAiPrefix))
      {
         // make sure we use the same base URL as the request, otherwise
         // we might not be allowed to display the redirected Ai content in the frame
         //
         // for example, RStudio Server might've been loaded from 'http://localhost:8787', but the
         // R ai server might try directing to 'http://127.0.0.1:8787'
         //
         // https://github.com/rstudio/rstudio/issues/13263
         std::string path = location.substr(rAiPrefix.length());
         std::string ref = request.headerValue("Referer");
         std::string redirect = fmt::format("{}ai/{}", ref, path);
                        ...
                        http::Response* pResponse)
{
   ...
   // markdown ai is also a special case
   if (path == "/doc/markdown_ai.html")
   {
      core::FilePath aiFile = options().rResourcesPath().completeChildPath(
         "markdown_ai.html");
      if (aiFile.exists())
      {
         pResponse->setFile(aiFile, request, filter);
         return;
      }
   }
   
   // roxygen ai
   if (path == "/doc/roxygen_ai.html")
   {
      core::FilePath aiFile = options().rResourcesPath().completeChildPath("roxygen_ai.html");
      if (aiFile.exists())
      {
         pResponse->setFile(aiFile, request, filter);
         return;
   ...
   {
      handleAiHomeRequest(request, kJsCallbacks, pResponse);
      return;
   }
...
// the ShowAi event will result in the Ai pane requesting the specified
// ai url. we handle this request directly by calling the R httpd function
// to dynamically form the correct http response
void handleAiRequest(const http::Request& request, http::Response* pResponse)
{
   handleHttpdRequest(kAiLocation,
                      boost::bind(r::sexp::findFunction, "httpd", "tools"),
                      request,
                      AiContentsFilter(request),
                      pResponse);
}
...
Error initialize()
{
   ...
   error = r::exec::RFunction(".rs.initAi", port, isDesktop).call(
                                                            &s_handleCustom);
   if (error)
      LOG_ERROR(error);
